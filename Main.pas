 ...
 FilePath := Trim(scEdit4.Text);
  ...
  try
    ...
    try
      ...
      if FileStream.Size > 50 * 1024 * 1024 then // 50 MB limit
      begin
        Exit; // File too large, exit silently for timer
      end;
      ...
      // Log('Sent file: ' + FileName + ' to all clients');
    finally
      FileStream.Free;
    end;
  except
    on E: Exception do
    begin
      // Log error but don't show message dialog for timer events
      // Log('Error in autoexec: ' + E.Message);
    end;
  end;
end;
